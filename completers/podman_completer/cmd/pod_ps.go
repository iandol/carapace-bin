...
func init() {
	...
	pod_psCmd.Flags().Bool("namespace", false, "Display namespace information of the pod")
	pod_psCmd.Flags().Bool("no-trunc", false, "Do not truncate pod and container IDs")
	pod_psCmd.Flags().Bool("noheading", false, "Do not print headers")
	pod_psCmd.Flags().BoolP("quiet", "q", false, "Print the numeric IDs of the pods only")
	pod_psCmd.Flags().String("sort", "created", "Sort output by created, id, name, or number")
