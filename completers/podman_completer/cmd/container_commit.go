...
func init() {
	...
	container_commitCmd.Flags().StringArrayP("change", "c", []string{}, "Apply the following possible instructions to the created image (default []): CMD | ENTRYPOINT | ENV | EXPOSE | LABEL | ONBUILD | STOPSIGNAL | USER | VOLUME | WORKDIR")
	container_commitCmd.Flags().StringP("format", "f", "oci", "`Format` of the image manifest and metadata")
