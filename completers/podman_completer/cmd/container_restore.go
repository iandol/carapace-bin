...
func init() {
	...
	container_restoreCmd.Flags().StringP("name", "n", "", "Specify new name for container restored from exported checkpoint (only works with --import)")
	container_restoreCmd.Flags().String("pod", "", "Restore container into existing Pod (only works with --import)")
